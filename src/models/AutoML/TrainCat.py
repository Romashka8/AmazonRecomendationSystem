        ...
        for i in tqdm.tqdm(range(min(n_stop, len(features)))):
            ...
            for f in features:
                if f not in res_features:
                    ...
                    current_cat_features = list(set(self.cat_features) & set(current_features)) if self.cat_features else None
                    ...
                    for dataset, dataset_name in zip(
                        ...
                        if dataset is not None:
                            predictions = model.predict_proba(dataset[current_features])[:, 1]
